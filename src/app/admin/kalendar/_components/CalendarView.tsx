...
import WeeklyCalendarTable, {
  getWeekStart,
  type TimeOff as CalendarTimeOff,
} from "@/components/WeeklyCalendarTable";
...
export default function CalendarView({
  ...
  const [slotForModal, setSlotForModal] = useState<{
    start: Date;
    end: Date;
  } | null>(null);
  const [appointmentToCancel, setAppointmentToCancel] =
    useState<AppointmentForCancel | null>(null);
  const [timeOffToDelete, setTimeOffToDelete] =
    useState<CalendarTimeOff | null>(null);
  const [isDeletingTimeOff, setIsDeletingTimeOff] = useState(false);
  const [isCancelling, setIsCancelling] = useState(false);
  const [selectionMode, setSelectionMode] = useState<
    "none" | "working_hours" | "time_off"
  >("none");
  const [selectedSlots, setSelectedSlots] = useState<Set<string>>(new Set());
  const [isSaving, setIsSaving] = useState(false);
  const [showReasonSelector, setShowReasonSelector] = useState(false);
  const [dayForWorkingHoursModal, setDayForWorkingHoursModal] =
    useState<Date | null>(null);
  const [showRegularScheduleModal, setShowRegularScheduleModal] =
    useState(false);
    ...
    () =>
      (appointments as any[])
        ...
        .map((a) => ({
          ...
          service: {
            title: a.service?.title ?? (a.isPause ? "PAUZA" : "Usluga"),
          },
          isPause: a.isPause,
        })),
  ...
  const { setContent: setHeaderSlotContent } = useAdminHeaderSlot() ?? {
    setContent: () => {},
  };
  ...
  useEffect(() => {
    ...
    const toolbarContent = (
      ...
      >
        {selectionMode === "none" ? (
          <>
            <div
              className="header-employees-container"
              style={{
                gap: "0.35rem",
          ...
          <div
            className="header-selection-container"
            style={{
              display: "flex",
              gap: "0.5rem",
              alignItems: "center",
          ...
          >
            <span style={{ fontWeight: 600, color: "var(--accent)" }}>
              {selectionMode === "working_hours"
                ? "IZBOR RADNOG VREMENA"
                : "IZBOR NERADNIH DANA"}
              {selectedSlots.size > 0 && ` (${selectedSlots.size})`}
            </span>
            <div
              style={{ marginLeft: "auto", display: "flex", gap: "0.75rem" }}
            >
              <button
                type="button"
                disabled={isSaving}
    ...
    employees,
    selectedEmployee,
    setHeaderSlotContent,
    selectionMode,
    selectedSlots,
    isSaving,
    defaultFieldDurationMinutes,
    router,
  ]);
        ...
        >
          ...
          >
            ...
            >
              ...
              {appointmentToCancel.service?.title &&
                ` · ${appointmentToCancel.service.title}`}
              {" · "}
              {format(
                typeof appointmentToCancel.startTime === "string"
                  ? new Date(appointmentToCancel.startTime)
        ...
        >
          ...
          >
            ...
            >
              ...
              {" · "}
              {format(
                typeof timeOffToDelete.date === "string"
                  ? new Date(timeOffToDelete.date)
                  : timeOffToDelete.date,
                "dd.MM.yyyy",
              )}
            </p>
            <div
      ...
      {showReasonSelector && (
        ...
        >
          ...
          >
            ...
            >
              {["Godišnji", "Bolovanje", "Praznik", "Slobodan dan"].map(
                (reason) => (
                  <button
                    key={reason}
                    type="button"
